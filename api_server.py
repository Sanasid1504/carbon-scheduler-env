...
async def reset(request: ResetRequest = None):
    ...
    try:
        ...
        print(f"[API] Task: {task_name}, Seed: {seed}")
        
        # Load config if not provided
        if config is None:
            config = load_task_config(task_name)
            print(f"[API] Loaded config from openenv.yaml")
        
        # Create environment with correct parameter name
        env_instance = SchedulerEnv(
            task_config=config,  # Correct parameter name
            seed=seed,
            use_real_carbon=False  # Use synthetic for determinism
        )
        current_task = task_name
        print(f"[API] Environment created")
        
        # Reset and get observation
        obs = env_instance.reset()
        print(f"[API] Environment reset complete")
...
async def step(request: StepRequest):
    ...
    try:
        print("[API] POST /step called")
        
        # Parse action
        action = Action(**request.action)
        print(f"[API] Action parsed: {len(action.schedule)} jobs scheduled")
        
        # Execute step
        obs, reward, done, info = env_instance.step(action)
        print(f"[API] Step executed: reward={reward.total:.4f}, done={done}")
        ...
        return JSONResponse(
            ...
            content={
                ...
                "done": done,
                "info": make_json_serializable(info)
            }
        )
    
    except Exception as e:
        print(f"[API] Error in /step: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_state():
    ...
    try:
        print("[API] GET /state called")
        state = env_instance.state()
        print(f"[API] State retrieved: step={state.episode_step}, done={state.done}")
